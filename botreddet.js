...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.cache.has(yetkilii)) return message.channel.send(` Bu Komudu Kullanabilmen İçin ${yetkilii} İdli Rolüne Sahip Olmalısın! Degilsen v!botlist-yetkili-rol @rol`)
  message.delete(5000);
  const sahip = args[0];
  if (!sahip)
    return message
